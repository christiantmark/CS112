...
public class LinkedTree {
    ...
    /* Returns a post iterator for this tree. */
    ...
    private class PostorderIterator implements LinkedTreeIterator {
        ...
        private void goToLeaf(Node n) {
            // The traversal starts with the left most node.
            if(n != null) {
            	Node travParent = n;
            	Node trav = n;
            	do {
        ...
        public int next() {
            ...
            int key = -1;							//initializes key
            
            if(nextNode.left == null && nextNode.right == null) {	//case 1: current node has no children
                lastVisited = nextNode;					//sets the current node to last visited
            	key = nextNode.key;
            	nextNode = nextNode.parent;
            }
            else if((nextNode.left != null && nextNode.left.key == lastVisited.key && nextNode.right == null) || (nextNode.right != null && nextNode.right.key == lastVisited.key && nextNode.left == null)) {
                lastVisited = nextNode;					//sets the current node to last visited
            	key = nextNode.key;
            	nextNode = nextNode.parent;
            }
            else if(nextNode.left != null && nextNode.left.key == lastVisited.key && nextNode.right != null) {	//case3a current node has 2 children and traversed back from left node
        		goToLeaf(nextNode.right);
                lastVisited = nextNode;					//sets the current node to last visited
        		key = nextNode.key;
        		nextNode = nextNode.parent;
            }
            else if(nextNode.right != null && nextNode.right.key == lastVisited.key && nextNode.left != null) {	//case3b current node has 2 children and traversed back from right node 
                lastVisited = nextNode;					//sets the current node to last visited
            	key = nextNode.key;
            	nextNode = nextNode.parent;
            }
    ...
    public static void main(String[] args) {
    	...
    	int[] t = {20, 12, 25, 7, 17, 23, 50, 16, 19, 40, 51, 15};
    	LinkedTree test = new LinkedTree();
    	test.insertKeys(t);
    	test.preorderPrint();
    	LinkedTreeIterator it = test.preorderIterator();
    	while(it.hasNext()) {
    		System.out.print(it.next() + " ");
    	}
    	System.out.println("");
    	test.postorderPrint();
    	LinkedTreeIterator iter = test.postorderIterator();
    	while(iter.hasNext()) {
